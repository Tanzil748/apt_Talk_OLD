...
CREATE TABLE posts (
    ...
    postAuthorId BIGINT REFERENCES users(id) ON DELETE CASCADE NOT NULL --foreign key
);
...
CREATE TABLE comments(
    ...
    commentUserId BIGINT REFERENCES users(id) ON DELETE CASCADE NOT NULL, --foreign key
    commentPostId BIGINT REFERENCES posts(id) ON DELETE CASCADE NOT NULL --foreign key
);

-- Bookmark Table
CREATE TABLE bookmarks(
    id BIGSERIAL NOT NULL PRIMARY KEY,
    bookmark_post_id BIGINT REFERENCES posts(id) ON DELETE CASCADE NOT NULL, --foreign key
    bookmark_user_id BIGINT REFERENCES users(id) ON DELETE CASCADE NOT NULL --foreign key
);
